    ...
    <> 
            {recipe.map((item,index)=>(
                <div className='grid' key={index}> 
                    <div className='pic'><img src={item.recipe.image} alt='image not found'/></div>
                    <div className='label'>{item.recipe.label}</div>
                    <div className='text'>{item.recipe.ingredients.map((data,index)=>(
                      <ul key={index}>
                          <li>{data.text}</li>
                      </ul>
                   ))}</div>
                   <div className='save-recipe' onClick={()=>saveRecipe(index,item.recipe.image,item.recipe.label,item.recipe.ingredients)}><button>Save</button></div>
                   </div>         
            ))}
    </>
  )
